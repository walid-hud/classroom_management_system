...
const ALL_SKILLS: available_skill[] = [
	"HTML",
	"JAVASCRIPT",
	"CSS",
	"NODEJS",
...
const VALID_LEVELS: skill_level[] = ["critical", "average", "good", "excellent"];

async function file_exists_path(path: string) {
	try {
		await access(path);
		return true;
	} catch (err: any) {
...
async function load_json_from_path(path: string): Promise<Student[] | null> {
			...
			if (item.skills !== undefined) {
				...
				for (const sk of item.skills) {
					if (
						...
						!ALL_SKILLS.includes(sk.name as available_skill) ||
						!VALID_LEVELS.includes(sk.level as skill_level)
					) {
...
async function prompt_and_load(manager: Class_Manger) {
...
class Class_Manger {
	...
	async mark_attendance() {
		...
		const student_id = await input({
			...
			required: true,
			validate: (v: string) => (this.data.find((s) => s.id === v) ? true : "student not found"),
		});
	...
	async show_valid_students() {
		...
		for (const s of valid) {
			log(
				`${s.id} - ${s.first_name} ${s.last_name} - skills: ${s.skills!.map((k) => `${k.name}(${k.level})`).join(
					", "
				)}`
			);
		}
...
async function read_json_file(path: string): Promise<Student[] | null> {
	...
	try {
		...
		log("read error:", (error as any)?.message ?? error);
		return null;
	}
}
...
async function write_json_file(path: string, data: Student[]) {
	...
	try {
		...
		log("write error:", (error as any)?.message ?? error);
	} finally {
		return success;
	}
}
...
async function add_skill(available_skills: available_skill[]): Promise<Skill[]> {
	let skills: Skill[] = [];
	while (true) {
